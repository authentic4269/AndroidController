...
import androidcontroller.AccelerometerService.DataBinder;
...
public class BluetoothService extends ServerConnection {
    ...
    private boolean connectDeviceToSocket(BluetoothDevice bt)
    {
    	try {
    		try {
	...
	public boolean connectToServer(String serverName) {
		...
		{
			if (connectDeviceToSocket(map.get(serverName))) {
				return true;
			}
			return false;
		}
